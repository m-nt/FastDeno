...
export class FastResponse {
	...
	json() {
		let response = `HTTP/1.1 ${this.status} ${status_to_string.get(this.status)}\n`
	...
	send(status:number, body:object, headers?:Map<string,string>){
		...
		if (!headers)
			return
		this.headers = new Map([...Array.from(this.headers.entries()),...Array.from(headers.entries())])
	}
}
...
export class FastDeno {
	...
	routes: Map<string, {(req: FastRequest, res: FastResponse, next:()=>void): object | void}[]> = new Map<string, {(req: FastRequest, res: FastResponse, next:()=>void): object | void}[]>();
	constructor() {}

	add_route(
		method: string,
		path: string,
		...callbacks: {(req: FastRequest, res: FastResponse, next:()=>void): object | void;}[]
	) {
	...
	async listen(port: number, address: string) {
		...
		while (true) {
			...
			const callbacks = this.routes.get(`${session.request.method}_${session.request.path}`)            
			if (!callbacks) {
				await client.write(this.encoder.encode(session.error(404, "Path didn't found")));
				client.close()
                continue
			}
			...
			session.response.headers.set("Content-Length",JSON.stringify(session.response.body).length.toString())
